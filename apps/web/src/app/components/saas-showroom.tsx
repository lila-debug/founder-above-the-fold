...
function Purchase({ onNext }: { onNext: () => void }) {
  return <section className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><article className="licence-card"><span className="cut-label bg-[#f4d13d]">One founder transformation · staging price</span><h2>CA$7,500</h2><p className="text-lg font-black uppercase">Built properly. Owned outright.</p><ul>{["Founder positioning and credibility repair", "Headline, About, Experience and Featured assembly", "Signal map, voice guide and 10 launch posts", "Private macOS and web operating workbench", "Assembly, handover and one revision round"].map((item) => <li key={item}><Check size={18} />{item}</li>)}</ul><Link className="hard-button bg-black text-white" href="/pricing">Open transformation panel <ArrowRight size={16} /></Link><button className="text-button" onClick={onNext} type="button"><RotateCcw size={15} /> Preview receipt state</button><p className="form-note"><LockKeyhole size={16} /> Stripe sandbox is selected. Checkout stays locked until its signed lifecycle is proved.</p></article><article className="paper-card p-5"><span className="section-kicker">01 → Inspect the transformation</span><h3 className="mt-4 text-3xl font-black uppercase">One outcome. No pricing maze.</h3><p className="mt-3 leading-7">The one-time transformation combines founder positioning, assembled profile assets, launch content, handover and the private operating workbench. The cabinet unlocks only after the signed server event creates both the access record and exactly one Mac licence; the return-page redirect cannot grant either.</p><div className="mt-6 grid gap-3"><div className="field-slat"><span>Commercial model</span><strong>One transformation</strong></div><div className="field-slat"><span>Payment rail</span><strong>Stripe staging locked</strong></div><div className="field-slat"><span>Auto renewal</span><strong>Off</strong></div></div></article></section>;
}
...
function Perks() { return <Panel title="Perks Hub" kicker="Unlocked with your licence"><div className="perk-grid">{[["90-day founder signal plan","12-panel workbook","Included"],["Voice jig library","English dialect + French region checks","Included"],["Public preview kit","Profile + links template","Included"],["Partner drawer","Clearly labelled offers","Optional"]].map(([title,body,label],i) => <article className={`perk-card perk-${i+1}`} key={title}><span>{String(i+1).padStart(2,"0")}</span><Gift size={28}/><h3>{title}</h3><p>{body}</p><strong>{label}</strong></article>)}</div></Panel> }
